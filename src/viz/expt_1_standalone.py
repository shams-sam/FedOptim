...
for j, (h_baseline, h_ours) in enumerate(zip(baselines, ours)):
    ...
    if loss_types[j] == 'ce':
        ln1b = ax1.plot(b_ep, b_acc, 'b', label='Vanilla-FL')
        ln1o = ax1.plot(o_ep, o_acc, 'r', label='LBGM-FL')
    else:
    ...
    ln2b = ax2.plot(b_ep, b_up, 'b', label='Vanilla-FL')
    ...
    ax1.legend(lns, labs, loc='upper right', fontsize=20
