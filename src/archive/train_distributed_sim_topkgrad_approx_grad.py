...
from utils import accumulate_grads_over_epochs, \
    accumulate_topk_grads_over_epochs, \
...
model_name = 'clf_{}_noise_{}_paradigm_{}_kgrad_{}_topk_{}' \
    '_lr_{}_decay_{}_batch_{}_distributed_sim'.format(
        args.clf, args.noise, args.paradigm, args.kgrads, args.topk,
...
for epoch in range(1, args.epochs + 1):
    ...
    grad_accum = accumulate_topk_grads_over_epochs([gradi], args.topk, device)
...
for i in x_ax:
    if i <= args.kgrads:
        ...
        u = (args.topk * args.kgrads) + (args.kgrads * (args.kgrads - i))
    d = num_params * i
    uplink.append(u)
    downlink.append(d)
    total.append(u+d)
